...
int MatrixMath::Invert(float* A, int n)
{
    ...
    // Serial.print("IMPORTANT: A[pivrow * n + k] = "); Serial.println(A[pivrow * n + k]);
		if (abs(A[pivrow * n + k]) < SING_MAT_LIMIT)
		{
			Serial.println("WARN: MatrixMath::Invert() : Inversion failed due to singular matrix");
			return 0;
		}
